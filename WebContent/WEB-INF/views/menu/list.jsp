...
<script type="text/javascript">
	...
	*/
	function selectIcon(){
			...
			$.ajax({
				...
				success:function(data){
					if(data.type == 'success'){
						...
						table += '</tr>';
						$("#icons-table").append(table);//将凭借的字符串显示在table中
					}else{
						$.messager.alert('信息提示',data.msg,'warning');
					}
				}
		...
		$('#select-icon-dialog').dialog({
            ...
            buttons: [{
                ...
                iconCls: 'icon-ok',
                handler: function(){
                	var icon = $(".selected a").attr('class');//获取确定的图标按钮
                	$("#add-icon").val(icon);
                	$("#edit-icon").val(icon);
	...
	function selected(e){
		$(".icon-td").removeClass('selected');//移除上一个的被选择的图标
		$(e).parent("td").addClass('selected');//重新添加新的选择的图标
	...
	*/
	$('#data-datagrid').treegrid({
		...
		columns:[[
			...
			{ field:'name',title:'菜单名称',width:100,sortable:true},/*sortable是否支持排序*/
